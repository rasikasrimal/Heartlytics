# Available functions (autogenerated): 81
# __init__: line 124
# hash: line 127
# verify: line 130
# check_needs_rehash: line 134
# encrypt: line 142
# decrypt: line 146
# encrypt_field: line 150
# decrypt_field: line 163
# get_keyring: line 166
# inject_branding: line 262
# inject_rbac_helpers: line 270
# inject_nav: line 278
# set_security_headers: line 283
# handle_forbidden: line 292
# load_research_paper: line 312
# replace_cites: line 340
# repl: line 341
# to_html: line 347
# enum_repl: line 350
# set_password: line 453
# check_password: line 457
# mfa_secret: line 480
# set_mfa_secret: line 502
# patient_data: line 652
# patient_data: line 676
# to_dict: line 725
# patient_name: line 749
# patient_name: line 772
# to_dict: line 805
# load_user: line 834
# enforce_session_timeout: line 840
# forbidden: line 854
# run_kmeans: line 986
# api_kmeans: line 1077
# load_model: line 1156
# admin_dashboard_alias: line 1199
# api_delete_prediction: line 1206
# api_delete_prediction_legacy: line 1221
# api_delete_all_predictions: line 1236
# api_delete_outliers: line 1249
# index: line 1269
# dashboard: line 1295
# outlier_handling: line 1308
# dashboard_pdf: line 1332
# dashboard_pdf_generate: line 1361
# fit_image: line 1510
# compute_cluster_colors: line 1591
# header_footer: line 1685
# dashboard_csv: line 1843
# dashboard_clean_csv: line 1875
# research_paper: line 1895
# report: line 1904
# _safe_read_csv: line 1913
# _winsorize_series: line 1922
# _strip_count_from_stats: line 1930
# build_eda_payload: line 1940
# _cramers_v: line 2080
# _make_upload_dir: line 2163
# _paths: line 2169
# _best_guess_mapping: line 2182
# norm: line 2184
# _save_map_payload: line 2209
# _load_map_payload: line 2215
# _apply_user_mapping: line 2223
# upload_form: line 2249
# upload_post: line 2258
# upload_columns_map: line 2309
# upload_columns_map_submit: line 2371
# upload_preprocess: line 2407
# upload_preprocess_task: line 2424
# upload_preprocess_finish: line 2500
# upload_eda: line 2536
# _missing: line 2560
# upload_preview: line 2679
# upload_download_clean: line 2701
# upload_predict: line 2711
# _int_or_none: line 2741
# upload_download_results: line 2846
# upload_bulk_pdf: line 2856
# roles: line 2919
# roles_set: line 2926
...
from services.pdf import generate_prediction_pdf, generate_dashboard_pdf
...
@login_required
def api_get_prediction(pid: int):  # Fetch full prediction record for detail view
    pred = Prediction.query.get(pid)
    if not pred:
        return jsonify({"ok": False, "error": "Not found"}), 404
    try:
        data = pred.to_dict() if hasattr(pred, "to_dict") else {
            "id": pred.id,
            "age": pred.age,
            "sex": pred.sex,
            "chest_pain_type": getattr(pred, "chest_pain_type", None),
            "resting_blood_pressure": getattr(pred, "resting_bp", None),
            "cholesterol": getattr(pred, "cholesterol", None),
            "max_heart_rate": getattr(pred, "max_heart_rate", None),
            "st_depression": getattr(pred, "oldpeak", None),
            "num_major_vessels": getattr(pred, "num_major_vessels", None),
            "thalassemia_type": getattr(pred, "thalassemia_type", None),
...
def outlier_handling():  # Handle outlier detection and management
    ...
    if selected and data:
        ...
        results = res
    # total outliers across all selected methods (after conversion to list-of-dicts)
    total_outliers = sum(len(v.get("rows", [])) for v in results.values()) if results else 0
...
def dashboard_pdf_generate():  # Generate and download PDF report
    ...
    try:
        ...
        sns = None
    if plt is not None:
        # Use transparent backgrounds so visuals blend with light/dark
        # surfaces. Colors for text and grids adapt by theme below.
        plt.rcParams.update(
            {
    ...
    return send_file(buf, as_attachment=True, download_name="predictions.pdf", mimetype="application/pdf")
    ...
    if plt is not None and sns is not None and not df.empty:
        ...
        buf_cm = io.BytesIO()
        fig.tight_layout()
        fig.savefig(buf_cm, format="PNG", dpi=150, transparent=True, facecolor="none", edgecolor="none")
        plt.close(fig)
        buf_cm.seek(0)
    ...
    if plt is not None and not df.empty and df["cluster_id"].notna().any():
        ...
        buf_kde = io.BytesIO()
        fig.tight_layout()
        fig.savefig(buf_kde, format="PNG", dpi=150, bbox_inches="tight", transparent=True, facecolor="none", edgecolor="none")
        plt.close(fig)
        buf_kde.seek(0)
...
def build_eda_payload(df: pd.DataFrame) -> dict:  # Build exploratory data analysis payload
    ...
    if "positive_probability" in df.columns:
        ...
        if vals.size:
            ...
            # Normal fit for a â€œbell-curveâ€ overlay, scaled to histogram area
            n = vals.size
            mu = float(vals.mean())
    ...
    # ----- categorical association matrix (CramÃ©râ€™s V between cats) -----
...
def upload_columns_map(uid: str):  # Display column mapping interface
    ...
    mapping_tips = {
        ...
        "num_major_vessels": "Tip: map ca here (0â€“4).",
    ...
    var_info = [
        ...
        {"var": "ca", "type": "Integer", "desc": "Major vessels colored by fluoroscopy (0â€“4)"},
...
def upload_preprocess_task(uid: str, task: str):  # Execute data preprocessing task
    ...
    df_orig = df.copy()
    if task == "dup":
        dup_df = df_orig[df_orig.duplicated()]
        if "patient_name" in dup_df.columns:
            dup_df = dup_df.drop(columns=["patient_name"])  # type: ignore[arg-type]
        dup_rows = dup_df.replace({np.nan: None}).to_dict(orient="records")

    # Build optional details per step
    details_by_col: dict[str, list[dict]] = {}
    def _rows_for_mask(mask, base_df):
        sub = base_df.loc[mask]
        if "patient_name" in sub.columns:
            sub = sub.drop(columns=["patient_name"])  # type: ignore[arg-type]
        return sub.replace({np.nan: None}).to_dict(orient="records")

    if task == "invalid":
        if "cholesterol" in df_orig.columns:
            mask = df_orig["cholesterol"] <= 0
            if mask.any(): details_by_col["cholesterol"] = _rows_for_mask(mask, df_orig)
        if "fasting_blood_sugar" in df_orig.columns:
            mask = ~df_orig["fasting_blood_sugar"].isin([0, 1])
            if mask.any(): details_by_col["fasting_blood_sugar"] = _rows_for_mask(mask, df_orig)
        if "exercise_induced_angina" in df_orig.columns:
            mask = ~df_orig["exercise_induced_angina"].isin([0, 1])
            if mask.any(): details_by_col["exercise_induced_angina"] = _rows_for_mask(mask, df_orig)
        if "st_depression" in df_orig.columns:
            mask = (df_orig["st_depression"] < 0) | (df_orig["st_depression"] > 7)
            if mask.any(): details_by_col["st_depression"] = _rows_for_mask(mask, df_orig)
        if "max_heart_rate_achieved" in df_orig.columns:
            mask = ~df_orig["max_heart_rate_achieved"].between(60, 250)
            if mask.any(): details_by_col["max_heart_rate_achieved"] = _rows_for_mask(mask, df_orig)
        if "resting_blood_pressure" in df_orig.columns:
            mask = ~df_orig["resting_blood_pressure"].between(80, 260)
            if mask.any(): details_by_col["resting_blood_pressure"] = _rows_for_mask(mask, df_orig)
        if "num_major_vessels" in df_orig.columns:
            mask = ~df_orig["num_major_vessels"].between(0, 4)
            if mask.any(): details_by_col["num_major_vessels"] = _rows_for_mask(mask, df_orig)
        for col, allowed in CATEGORICAL_ALLOWED.items():
            if col in df_orig.columns:
                bad_mask = ~df_orig[col].isin(allowed)
                if bad_mask.any(): details_by_col[col] = _rows_for_mask(bad_mask, df_orig)

    if task == "impute":
        for col in list(NUMERIC_FEATURES_INT | NUMERIC_FEATURES_FLOAT) + [
            "sex", "fasting_blood_sugar", "exercise_induced_angina",
            "chest_pain_type", "Restecg", "st_slope_type", "thalassemia_type",
        ]:
            if col in df_orig.columns and df_orig[col].isna().any():
                details_by_col[col] = _rows_for_mask(df_orig[col].isna(), df_orig)

    if task == "outliers":
        for col in (NUMERIC_FEATURES_INT | NUMERIC_FEATURES_FLOAT):
            if col in df_orig.columns:
                s = pd.to_numeric(df_orig[col], errors="coerce").astype(float)
    ...
    for m in clog:
        ...
        if task == "dup" and dup_rows:
            ...
            if ":" in m:
                col_prefix = m.split(":", 1)[0]
                det = details_by_col.get(col_prefix)
                if det:
                    entry["details"] = det
...
def upload_bulk_pdf(uid: str):
    ...
    for _, r in df.iterrows():
        ...
        lines = [
            ...
            f"FBS â‰¥120 mg/dL: {YESNO.get(int(r['fasting_blood_sugar']), r['fasting_blood_sugar'])}",
            ...
            f"Positive Probability: {'â€”' if pos_prob_pct is None else f'{pos_prob_pct}%'}",
            f"Risk Band: {risk_band or 'â€”'}",
